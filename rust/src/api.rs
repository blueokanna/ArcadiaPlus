...
use crate::crypto::{Aead, Aes128Gcm, Aes256Gcm};
use crate::error::{Result, VeloGuardError};
use crate::logging::FlutterLogLayer;
use crate::types::*;
use crate::{VELOGUARD_INSTANCE, get_veloguard_instance};
...
fn init_tracing_safe() -> std::result::Result<(), ()> {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(
            "debug,crate::core=debug,veloguard_lib=debug,crate::netstack=debug,tokio=warn,rustls=warn"
        ));
    ...
    #[cfg(target_os = "android")]
    {
        let android_layer = tracing_android::layer("VeloGuard").map_err(|error| {
            crate::core::logging::add_log(format!(
        ...
        if result.is_ok() {
            tracing::info!(
                "VeloGuard FFI bridge initialized (Android) - logs visible in logcat and Flutter UI"
            );
        }
...
pub async fn start_proxy_from_yaml(yaml_config: String) -> std::result::Result<(), String> {
    ...
    let veloguard = crate::core::VeloGuard::new(config)
        .await
...
pub async fn stop_proxy() -> std::result::Result<(), String> {
    ...
    let tracker = crate::core::connection_tracker::global_tracker();
    tracker.reset();
    {
...
pub async fn get_proxies() -> std::result::Result<Vec<ProxyInfoDto>, String> {
    ...
    if let Some(veloguard) = guard.as_ref() {
        ...
        for outbound_config in &config.outbounds {
            ...
            let proxy = outbound_manager.get_proxy(tag);

            let (server, port) = match &proxy {
                Some(proxy) => proxy
                    .server_addr()
                    .map(|(s, p)| (Some(s), Some(p)))
                    .unwrap_or((outbound_config.server.clone(), outbound_config.port)),
                None => (outbound_config.server.clone(), outbound_config.port),
            };

            // Proxy groups know whether the member they would use is up; leaf
            // outbounds only find out when traffic flows, so they keep the
            // optimistic defaults until `test_*_latency` says otherwise.
            let (alive, latency_ms) = proxy
                .as_ref()
                .and_then(|proxy| proxy.health_snapshot())
                .map(|(alive, latency)| (alive, latency.map(|d| d.as_millis() as u64)))
                .unwrap_or((true, None));
    ...
    if let Some(veloguard) = guard.as_ref() {
        ...
        for outbound in &config.outbounds {
            let group_type = match outbound.outbound_type {
                crate::core::OutboundType::Selector => "selector",
                crate::core::OutboundType::Urltest => "url-test",
                crate::core::OutboundType::Fallback => "fallback",
                crate::core::OutboundType::Loadbalance => "load-balance",
                crate::core::OutboundType::Relay => "relay",
                _ => continue,
            };
...
pub async fn set_proxy_mode(mode: i32) -> std::result::Result<(), String> {
    crate::core::set_runtime_proxy_mode(mode);
...
pub async fn get_proxy_mode() -> std::result::Result<i32, String> {
    Ok(crate::core::get_runtime_proxy_mode())
}
...
pub async fn initialize_veloguard(config_json: String) -> Result<()> {
    ...
    let config: VeloGuardConfig = nextjson::from_str(&config_json)
    ...
    let veloguard = crate::core::VeloGuard::new(core_config)
        .await
...
pub async fn test_config(config_json: String) -> Result<bool> {
    ...
    let core_config = convert_ffi_config_to_core(config)?;
    match crate::core::VeloGuard::new(core_config).await {
        Ok(_) => Ok(true),
        Err(_) => Ok(false),
    }
}
...
pub async fn get_logs(lines: Option<u32>) -> Result<Vec<String>> {
    ...
    let logs = crate::core::logging::get_recent_logs(max_lines);
    if logs.is_empty() {
        Ok(vec![
...
fn configure_rule_providers(config_json: &str) -> Result<()> {
    use crate::core::rule_provider::{RuleProviderConfig, RuleProviderType};
    ...
    crate::core::set_runtime_rule_providers(providers);
    Ok(())
}
...
fn parse_inbound_type(value: &str, tag: &str) -> Result<crate::core::InboundType> {
    use crate::core::InboundType;
...
fn parse_outbound_type(value: &str, tag: &str) -> Result<crate::core::OutboundType> {
    use crate::core::OutboundType;
...
fn parse_rule_type(value: &str) -> Result<crate::core::RuleType> {
                ...
                if let Some(veloguard) = guard.as_ref() {
                    ...
                    for inbound in &config.inbounds {
                        if matches!(
                            ...
                            crate::core::InboundType::Mixed | crate::core::InboundType::Http
                        ) {
                            port = inbound.port;
                            break;
                        }
...
pub async fn test_shadowsocks_latency(
    ...
    if let Err(e) = crate::crypto::fill_random(&mut client_salt) {
        return Ok(LatencyTestResult {
            proxy_name,
            latency_ms: None,
            success: false,
...
impl SsAeadCipher {
    fn new(key: &[u8]) -> std::result::Result<Self, String> {
        let inner = if key.len() == 32 {
            SsAeadCipherInner::Aes256Gcm(
                Aes256Gcm::new_from_slice(key).map_err(|_| "Invalid AES-256 key")?,
            )
        } else if key.len() == 16 {
            SsAeadCipherInner::Aes128Gcm(
                Aes128Gcm::new_from_slice(key).map_err(|_| "Invalid AES-128 key")?,
            )
        } else {
    ...
    /// Little-endian counter nonce, as Shadowsocks AEAD specifies.
...
/// Send `GET <url>` through the local HTTP proxy and read the status line.
///
/// A forward proxy expects the absolute URI on the request line, so this is
/// exactly the exchange our own HTTP inbounds already serve. Speaking it here
/// keeps the probe independent of any client library — and keeps the third
/// HTTP stack out of the product. Only the status line is read: the probe is a
/// `generate_204`, whose body never arrives.
async fn test_via_http_proxy(
    ...
    let parsed = corduit::common::url::Url::parse(url)
        .map_err(|error| format!("Invalid test URL '{url}': {error}"))?;
    let host = parsed
        .host_str()
        .ok_or_else(|| format!("Test URL '{url}' has no host"))?
        .to_string();

    let request = format!(
        "GET {url} HTTP/1.1\r\nHost: {host}\r\nUser-Agent: VeloGuard/{}\r\nAccept: */*\r\nProxy-Connection: close\r\nConnection: close\r\n\r\n",
        env!("CARGO_PKG_VERSION")
    );
    ...
    let exchange = async {
        let mut stream = tokio::net::TcpStream::connect(proxy_addr)
            .await
            .map_err(|error| format!("Connect to proxy failed: {error}"))?;
        stream.set_nodelay(true).ok();
        stream
            .write_all(request.as_bytes())
            .await
            .map_err(|error| format!("Write through proxy failed: {error}"))?;

        // Read exactly one line: the status line.
        ...
        while line.len() < 512 {
            let read = stream
                .read(&mut byte)
                .await
                .map_err(|error| format!("Read from proxy failed: {error}"))?;
            if read == 0 {
                break;
            }
            line.push(byte[0]);
        ...
        let _version = parts
            .next()
            .ok_or_else(|| "Proxy closed the connection without a response".to_string())?;
        let code: u16 = parts
            .next()
            .ok_or_else(|| format!("Malformed proxy response: {trimmed}"))?
            .parse()
            .map_err(|_| format!("Malformed status code in proxy response: {trimmed}"))?;

        Ok::<u16, String>(code)
    };

    let code = tokio::time::timeout(timeout, exchange)
        .await
...
pub async fn enable_tun_mode_with_mode(mode: String) -> Result<TunStatus> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match crate::netstack::ensure_wintun().await {
            ...
            Err(e) => {
                return Ok(TunStatus {
                    ...
                    mtu: None,
                    error: Some(format!(
                        "Failed to load wintun.dll: {}. Please ensure you have administrator privileges.",
                        e
                    )),
                });
            }
        ...
        crate::set_windows_tun_device(tun);
        crate::core::set_runtime_proxy_mode(mode_int);
...
pub fn get_windows_proxy_mode_str() -> String {
    match crate::core::get_runtime_proxy_mode() {
        1 => "global".to_string(),
        2 => "direct".to_string(),
        3 => "rule".to_string(),
        _ => "rule".to_string(),
...
mod config_conversion_tests {
    ...
    fn supported_aliases_are_explicit() {
        assert_eq!(
            ...
            crate::core::OutboundType::Hysteria2
        );
        assert_eq!(
            parse_outbound_type("socks", "proxy").unwrap(),
            crate::core::OutboundType::Socks5
        );
    }
    ...
    fn local_proxy_endpoint_follows_inbound_address_family() {
        use crate::core::{InboundConfig, InboundType};
        use std::net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr};
