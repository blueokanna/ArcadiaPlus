...
/// Set up the socket protect callback used by the netstack layer
fn setup_protect_callback() {
    android_log(
        "INFO",
        "Setting up socket protect callback for the netstack layer",
    );
    ...
    android_log(
        "INFO",
        "Socket protect callback configured for the netstack layer",
    );
    info!("Socket protect callback configured for the netstack layer");
}
