...
//!
//! ```rust,no_run
//! use rust_lib_veloguard::dns::{DnsManager, DnsConfig, RecordType};
//!
//! #[tokio::main]
//! async fn main() -> rust_lib_veloguard::dns::Result<()> {
...
/// Bogon / special-purpose address classification.
///
/// The CIDR tables and the classification rules live in `corduit` (the engine
/// crate); this module only re-exports them so callers keep a stable
/// `crate::dns::bogon` path and we do not carry two copies of the same tables.
pub mod bogon {
    pub use corduit::dns::bogon::*;
}
pub mod cache;
pub mod client;
pub mod config;
...
pub use bogon::{
    BogonType, classify_bogon, contains_bogon, filter_bogons, is_bogon, is_bogon_ipv4,
    is_bogon_ipv6, is_loopback, is_private, is_reserved,
};
pub use cache::{CacheEntry, CacheStats, DnsCache};
pub use client::{DnsClient, DnsProtocol, create_clients};
