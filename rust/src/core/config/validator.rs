        ...
        for rule in rules {
            ...
            match rule.rule_type {
                RuleType::Domain | RuleType::DomainSuffix => {
                    // Fail closed: a payload that is not a syntactically valid
                    // DNS name can never match once traffic is flowing, so it is
                    // rejected here instead of silently falling through to the
                    // default outbound. Suffixes may be written with a leading
                    // dot (".example.com"); `Name` rejects the empty label, so
                    // normalise that one spelling first.
                    let candidate = rule.payload.trim_start_matches('.');
                    if candidate.is_empty() {
                        return Err(Error::config("Domain rule payload cannot be empty"));
                    }
                    recurse_x::name::Name::from_ascii(candidate).map_err(|err| {
                        Error::config(format!(
                            "Invalid domain rule payload '{}': {}",
                            rule.payload, err
                        ))
                    })?;
                }
                RuleType::DomainKeyword => {
                    if rule.payload.is_empty() {
                        return Err(Error::config("Domain keyword payload cannot be empty"));
                    }
                }
                RuleType::DomainRegex => {
                    // A broken pattern used to surface as a per-request match
                    // failure; compile it once here so misconfiguration fails
                    // while the config is still being loaded.
                    regex::Regex::new(&rule.payload).map_err(|err| {
                        Error::config(format!("Invalid domain regex '{}': {}", rule.payload, err))
                    })?;
                }
