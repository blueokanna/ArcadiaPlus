...
impl ConnectionTracker {
    ...
    pub fn finalize(self) -> TrafficCounters {
        TrafficCounters {
...
pub struct TrafficStatsManager {
    global_stats: RwLock<TrafficCounters>,
    proxy_stats: DashMap<String, TrafficCounters>,
...
impl TrafficStatsManager {
    ...
    pub fn new() -> Self {
        Self {
            global_stats: RwLock::new(TrafficCounters::default()),
    ...
    pub fn global_stats(&self) -> TrafficCounters {
        match self.global_stats.try_read() {
            Ok(guard) => guard.clone(),
            Err(_) => TrafficCounters::default(),
        }
    }

    /// Get traffic statistics for a specific proxy
    pub fn proxy_stats(&self, proxy_tag: &str) -> Option<TrafficCounters> {
        self.proxy_stats.get(proxy_tag).map(|s| s.clone())
    }

    /// Get all proxy statistics
    pub fn all_proxy_stats(&self) -> Vec<(String, TrafficCounters)> {
        self.proxy_stats
            .iter()
    ...
    pub async fn reset(&self) {
        *self.global_stats.write().await = TrafficCounters::default();
...
pub struct TrafficSummary {
    pub global_stats: TrafficCounters,
    pub proxy_count: usize,
    pub active_connections: usize,
    pub proxy_stats: Vec<(String, TrafficCounters)>,
}
