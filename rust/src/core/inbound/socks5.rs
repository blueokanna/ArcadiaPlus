...
impl Socks5Inbound {
    ...
    async fn handle_connection(
        ...
        // SOCKS5 handshake (greeting + credentials when configured)
    ...
    async fn perform_handshake(stream: &mut tokio::net::TcpStream, auth: &AuthStore) -> Result<bool> {
        match auth.socks5_greeting(stream).await? {
            ...
            // greeting already answered the client in the latter case.
            Socks5Greeting::NotSocks5 | Socks5Greeting::NoAcceptableMethod => Ok(false),
        }
    }
