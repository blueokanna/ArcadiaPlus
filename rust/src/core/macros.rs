...
macro_rules! bail_network {
    ($($arg:tt)*) => {
        return Err($crate::core::error::Error::Network {
            message: format!($($arg)*),
            source: None,
        })
    };
...
macro_rules! bail_protocol {
    ($($arg:tt)*) => {
        return Err($crate::core::error::Error::Protocol {
            message: format!($($arg)*),
            protocol: None,
            source: None,
        })
...
macro_rules! bail_dns {
    ($($arg:tt)*) => {
        return Err($crate::core::error::Error::Dns {
            message: format!($($arg)*),
            source: None,
        })
    };
...
macro_rules! try_network {
    ($expr:expr) => {
        $expr.map_err(|e| $crate::core::error::Error::Network {
            message: e.to_string(),
            source: None,
        })?
    };
...
macro_rules! try_protocol {
    ($expr:expr) => {
        $expr.map_err(|e| $crate::core::error::Error::Protocol {
            message: e.to_string(),
            protocol: None,
            source: None,
        })?
...
macro_rules! try_dns {
    ($expr:expr) => {
        $expr.map_err(|e| $crate::core::error::Error::Dns {
            message: e.to_string(),
            source: None,
        })?
    };
...
macro_rules! try_config {
    ($expr:expr) => {
        $expr.map_err(|e| $crate::core::error::Error::Config {
            message: e.to_string(),
            source: None,
        })?
    };
...
macro_rules! impl_outbound_proxy_trait {
    ($name:ident { tag: $tag:expr $(,)? }) => {
        ...
        impl $crate::core::outbound::OutboundProxy for $name {
            fn tag(&self) -> &str {
                &$tag
            }
            ...
            async fn connect(&self) -> $crate::core::error::Result<()> {
                Ok(())
            }

            async fn disconnect(&self) -> $crate::core::error::Result<()> {
                Ok(())
            }
        }
    };
...
macro_rules! impl_transport {
    ($name:ident { config: $config_type:ty $(,)? }) => {
        impl $name {
            pub fn new(config: $config_type) -> $crate::core::error::Result<Self> {
                Ok(Self { config })
            }
...
macro_rules! impl_transport_wrap {
    ($self:expr, $stream:expr, $wrap_fn:expr, $transport_name:expr) => {
        $wrap_fn($self, $stream)
            .await
            .map_err(|e| $crate::core::error::Error::Network {
...
macro_rules! connect_with_timeout {
    ($addr:expr, $timeout:expr) => {
        tokio::time::timeout($timeout, tokio::net::TcpStream::connect($addr))
            .await
            .map_err(|_| $crate::core::error::Error::Timeout {
...
macro_rules! relay_streams {
    ($inbound:expr, $outbound:expr) => {
        $crate::core::outbound::relay_bidirectional_with_connection(
            $inbound,
            $outbound,
            $crate::core::connection_tracker::global_tracker(),
            None,
        )
        .await
    };
