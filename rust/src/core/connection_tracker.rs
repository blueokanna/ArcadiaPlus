...
///
/// A `Lazy` rather than a `static mut` constructor: the tracker owns atomics
/// and dash maps and must be built exactly once, on first use, so that a
/// counter read during startup cannot race its own initialization.
