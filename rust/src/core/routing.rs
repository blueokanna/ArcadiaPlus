use crate::core::config::{Config, Mode, RuleConfig, RuleType};
use crate::core::error::{Error, Result};
use crate::core::geoip::{GeoIpManager, is_local_or_private_ip};
use crate::core::rule_provider::RuleProviderConfig;
use crate::core::rule_provider::RuleProviderManager;
use ipnet::IpNet;
use lru::LruCache;
use regex::Regex;
...
impl Router {
    pub async fn new(config: Arc<RwLock<Config>>) -> Result<Self> {
        // Outbounds and rule providers can build TLS clients from here on, so
        // the process-wide provider has to exist before the first one does.
        crate::core::tls::install_crypto_provider();
    ...
    pub async fn match_outbound(
        ...
        let (effective_mode, direct_outbound, global_outbound, default_outbound) = {
            ...
            let direct_outbound = config
                ...
                .iter()
                .find(|outbound| {
                    outbound.outbound_type == crate::core::config::OutboundType::Direct
                })
            ...
            let global_outbound = config
                ...
                .find(|outbound| {
                    matches!(
                        ...
                        crate::core::config::OutboundType::Selector
                            | crate::core::config::OutboundType::Urltest
                            | crate::core::config::OutboundType::Fallback
                            | crate::core::config::OutboundType::Loadbalance
                    )
                })
                .or_else(|| {
                    config.outbounds.iter().find(|outbound| {
                        !matches!(
                            outbound.outbound_type,
                            crate::core::config::OutboundType::Direct
                                | crate::core::config::OutboundType::Reject
                        )
                    })
                })
                .map(|outbound| outbound.tag.clone());
    ...
    proptest! {
        ...
        fn prop_rules_match_in_priority_order(
            ...
            let config = Config {
                rules: rules.clone(),
                outbounds: vec![
                    crate::core::config::OutboundConfig {
                        outbound_type: crate::core::config::OutboundType::Direct,
                        tag: "first".to_string(),
                        server: None,
                        port: None,
