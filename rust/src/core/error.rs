...
impl From<crate::protocol::ProtocolError> for Error {
    /// Keep the protocol layer's classification instead of flattening it.
    ///
    /// "the certificate did not verify", "the peer closed the connection" and
    /// "the configuration asked for something impossible" send the user to three
    /// different places, so the protocol's own distinction survives the trip up
    /// into the engine's error type.
    fn from(error: crate::protocol::ProtocolError) -> Self {
        use crate::protocol::ProtocolError;

        match error {
            ProtocolError::Io(error) => Error::Io(error),
            ProtocolError::InvalidConfig(message) => Error::Config {
                message,
                source: None,
            },
            ProtocolError::Tls(message) => Error::Tls {
                message,
                source: None,
            },
            ProtocolError::Crypto(message) => Error::Tls {
                message: format!("crypto: {message}"),
                source: None,
            },
            ProtocolError::AuthFailed => Error::Auth {
