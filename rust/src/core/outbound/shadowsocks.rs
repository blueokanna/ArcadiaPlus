...
use crate::crypto::{Aead, Aes128Gcm, Aes256Gcm, ChaCha20Poly1305, Hkdf, Sha1};
...
impl OutboundProxy for ShadowsocksOutbound {
    ...
    async fn test_http_latency(
        ...
        crate::crypto::fill_random(&mut client_salt)
        ...
        let target = crate::core::outbound::TargetAddr::Domain(host.clone(), url_port);
    ...
    async fn relay_tcp_with_connection(
        ...
        use crate::core::connection_tracker::global_tracker;
...
fn derive_subkey_2022(password: &str, salt: &[u8], key_len: usize) -> Result<Vec<u8>> {
    use crate::crypto::base64::Engine as _;
    ...
    let mut hasher = crate::crypto::Blake3::new_derive_key(b"shadowsocks 2022 session subkey");
...
impl AeadCipher {
    ...
    /// Shadowsocks AEAD runs the nonce counter as a little-endian integer in
    /// the first 8 bytes of a 12-byte nonce, so each direction is a single,
    /// monotonic keystream index. The counter is never reused: both peers
    /// rekey long before a wrap could happen.
...
/// Send one encrypted chunk: `[encrypted 2-byte length][encrypted payload]`.
...
/// Receive and decrypt one chunk; `None` means a clean EOF.
async fn recv_decrypted_chunk<R: tokio::io::AsyncRead + Unpin>(
    ...
    // Encrypted length field, plus its tag.
    ...
    // Encrypted payload, plus its tag.
...
///
/// UDP packets each carry a fresh salt and use a zero nonce, because the
/// subkey derived from that salt is what makes the nonce unique.
...
mod tests {
    ...
    fn test_address_header_ipv4() {
        let config = OutboundConfig {
            tag: "test".to_string(),
            outbound_type: crate::core::config::OutboundType::Shadowsocks,
...
mod property_tests {
    ...
    proptest! {
        ...
        /// **Feature: rust-codebase-optimization, Property: encrypt/decrypt round-trip**
        ...
        /// **Feature: rust-codebase-optimization, Property: UDP encrypt/decrypt round-trip**
