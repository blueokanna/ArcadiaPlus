...
/// The congestion controllers a TUIC server understands.
///
/// Re-exported so `core::outbound::tuic::CongestionControl` keeps naming the
/// same type: the policy lives in `protocol::quic_client` now, because TUIC is
/// not the only outbound that has to choose one.
pub use crate::protocol::quic_client::CongestionControl;
...
/// Payload budget for one TUIC datagram, leaving room for the message header.
...
impl TuicConnection {
    ...
    ///
    /// Native mode fragments payloads that do not fit one datagram (the
    /// `frag_total` / `frag_id` fields are computed here); QUIC mode writes one
    ...
    pub async fn send_udp_packet(
        ...
        match self.udp_relay_mode {
            UdpRelayMode::Native => {
                ...
                // sending fragment 0 of 1 for a large payload would have the
                // server see a truncated datagram.
            ...
            UdpRelayMode::Quic => {
                ...
                // QUIC mode carries one message per stream, so every message is
                // the whole packet: `frag_total = 1`, `frag_id = 0`.
                buf.put_u8(1);
                buf.put_u8(0);
    ...
    pub async fn recv_udp_packet(&self) -> Result<(u16, TargetAddr, Vec<u8>)> {
        loop {
            let (assoc_id, frag_total, frag_id, target, payload) = match self.udp_relay_mode {
                UdpRelayMode::Native => {
                    let datagram = self.connection.recv_datagram().await.map_err(|e| {
            ...
            if let Some((target, full)) =
                assembler.add(assoc_id, frag_id, frag_total, target, payload)
            {
                return Ok((assoc_id, target, full));
            }
        }
    }
...
/// Incomplete packets held at once, across every association.
const MAX_PENDING_FRAGMENTS: usize = 64;

/// Reassembly buffer for fragmented TUIC UDP packets, keyed by association id
/// (TUIC v5: every fragment of one packet shares the same `assoc_id`).
...
impl TuicFragAssembler {
    ...
    fn add(
        ...
        if entry.1.len() >= usize::from(entry.0) {
            let (_, fragments, target, _) = self.pending.remove(&assoc_id)?;
...
impl TuicOutbound {
    pub fn new(config: OutboundConfig) -> Result<Self> {
        ...
        let congestion_control = match config
            .options
            .get("congestion-controller")
            .or_else(|| config.options.get("congestion_control"))
            .map(yaml_value_to_string)
        {
            Some(value) => value.parse::<CongestionControl>().map_err(|()| {
                Error::config(format!(
                    "TUIC outbound '{}': unknown congestion control '{value}' \
                     (expected bbr, cubic or new_reno)",
                    config.tag
                ))
            })?,
        ...
        if zero_rtt_handshake {
            warn!(
                "TUIC outbound '{}': zero-rtt-handshake / reduce-rtt is not supported by the \
        ...
        if disable_sni {
            warn!(
                "TUIC outbound '{}': disable-sni is not supported by the in-repo QUIC transport \
                 (the certificate name is always sent as SNI); the option is ignored",
                config.tag
            );
        }
        ...
        // The QUIC policy is built before `TuicConfig` takes ownership of the
        // values it shares with it.
        let quic_tuning = QuicClientTuning {
            alpn: alpn.clone(),
            skip_cert_verify,
            congestion: congestion_control,
            keep_alive: Some(Duration::from_millis(heartbeat)),
            ..Default::default()
        };
    ...
    async fn get_or_create_connection(&self) -> Result<Arc<TuicConnection>> {
        ...
        // `sni` — or the server host — is what gets verified.
        ...
        *conn_guard = Some(Arc::clone(&tuic_conn));
    ...
    pub async fn relay_udp(&self, target: &TargetAddr, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let assoc_id: u16 = crate::crypto::random_u16();

        conn.send_udp_packet(assoc_id, target, data).await?;
...
mod tests {
    ...
    #[test]
    fn udp_message_roundtrip_carries_the_fragment_fields() {
        let addr = encode_address_bytes(&TargetAddr::Domain("example.com".to_string(), 53))
            .expect("address");
        let msg = build_udp_message(7, 3, 1, &addr, b"chunk").expect("message");

        let (assoc_id, frag_total, frag_id, target, payload) =
            parse_udp_packet(&msg).expect("parsed");
    ...
    #[test]
    fn udp_packet_rejects_truncated_payloads() {
        let addr = encode_address_bytes(&TargetAddr::Domain("example.com".to_string(), 53))
            .expect("address");
        let mut msg = build_udp_message(7, 1, 0, &addr, b"longer").expect("message");
        msg.truncate(msg.len() - 2);
        // The u16 length field promises more bytes than the datagram carries.
        assert!(parse_udp_packet(&msg).is_err());
    }
    ...
    #[test]
    fn t_uic_fragment_buffer_stays_bounded() {
        let mut assembler = TuicFragAssembler::new();
        let target = TargetAddr::Domain("example.com".to_string(), 53);
        for assoc in 0..200u16 {
            assembler.add(assoc, 0, 2, target.clone(), b"x".to_vec());
        }
        assert!(
    ...
    fn test_tuic_outbound_new() {
        ...
        let config = OutboundConfig {
            ...
            outbound_type: crate::core::config::OutboundType::Tuic,
