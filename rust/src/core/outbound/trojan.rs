...
use crate::crypto::{Digest, Sha224};
use std::sync::Arc;
...
fn compute_password_hash(password: &str) -> [u8; 56] {
    ...
    let hex_str = crate::crypto::hex::encode(result);
    let mut hash = [0u8; 56];
...
mod tests {
    ...
    fn test_password_hash_known_value() {
        ...
        let mut hasher = Sha224::new();
        hasher.update(password.as_bytes());
        let expected = crate::crypto::hex::encode(hasher.finalize());
    ...
    fn test_trojan_outbound_new() {
        ...
        let config = OutboundConfig {
            ...
            outbound_type: crate::core::config::OutboundType::Trojan,
