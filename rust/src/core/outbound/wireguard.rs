...
use crate::protocol::wireguard::{WireGuardTunnel, public_key_from_private};
...
impl WireguardOutbound {
    ...
    pub async fn relay_udp(&self, target: &TargetAddr, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let src_port = 40000 + (crate::crypto::random_u16() % 20000);
        ...
        packet[4..6].copy_from_slice(&crate::crypto::random_u16().to_be_bytes());
        packet[6] = 0x40;
        packet[7] = 0x00;
        packet[8] = 64;
        packet[9] = 17;
...
impl OutboundProxy for WireguardOutbound {
    ...
    async fn relay_tcp_with_connection(
        ...
        let mut seq: u32 = crate::crypto::random_u32();
...
fn decode_base64_key(s: &str) -> std::result::Result<[u8; 32], String> {
    use crate::crypto::base64::{Engine as _, STANDARD};
...
mod tests {
    ...
    fn test_wireguard_outbound_new() {
        ...
        let config = OutboundConfig {
            ...
            outbound_type: crate::core::config::OutboundType::Wireguard,
