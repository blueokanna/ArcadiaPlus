...
pub struct VmessOutbound {
    ...
    // QUIC-specific fields: one connection is reused, every VMess exchange
    // rides its own bidirectional stream on top of it.
    quic_connection: Mutex<Option<Arc<quic_client::QuicConnection>>>,
...
impl VmessOutbound {
    ...
    pub fn generate_request_key(&self) -> [u8; 16] {
        let mut key = [0u8; 16];
        crate::crypto::fill_random(&mut key).expect("Failed to generate random key");
        key
    }

    pub fn generate_request_iv(&self) -> [u8; 16] {
        let mut iv = [0u8; 16];
        crate::crypto::fill_random(&mut iv).expect("Failed to generate random IV");
        iv
    }
    ...
    pub fn seal_header(&self, header: &VmessHeader, timestamp: i64) -> Result<Vec<u8>> {
        ...
        if header.padding_length > 0 {
            ...
            crate::crypto::fill_random(&mut padding).ok();
        ...
        let cipher = Aes128Gcm::new(&header_key);
        ...
        let length_cipher = Aes128Gcm::new(&header_length_key);
    ...
    async fn connect_stream(&self) -> Result<Box<dyn AsyncReadWrite>> {
        match self.transport {
            VmessTransport::Ws => {
                ...
                // One WebSocket implementation for the whole crate: the shared
                // transport owns the handshake (including the accept-key check)
                // and RFC 6455 framing, so VMess cannot drift away from what
                // the other outbounds speak.
                let transport = WebSocketTransport::new(
                    WebSocketConfig {
                        path: ws_opts.path.clone(),
                        host: ws_opts.host.clone(),
                        headers: ws_opts.headers.clone(),
                        ..Default::default()
                    },
                    &self.server,
                    self.port,
                ...
                if self.tls_enabled {
                    ...
                    let ws_stream = transport.connect(tls_stream).await.map_err(|error| {
                        Error::protocol(format!("WebSocket handshake failed: {error}"))
                    })?;
                    Ok(Box::new(ws_stream) as Box<dyn AsyncReadWrite>)
                } else {
                    let tcp_stream = self.connect_tcp().await?;
                    let ws_stream = transport.connect(tcp_stream).await.map_err(|error| {
                        Error::protocol(format!("WebSocket handshake failed: {error}"))
                    })?;
                    Ok(Box::new(ws_stream) as Box<dyn AsyncReadWrite>)
                }
    ...
    async fn connect_quic(&self) -> Result<quic_client::QuicStream> {
        // Reuse a live connection: every VMess exchange rides its own
        // bidirectional stream, so keeping the connection saves a full QUIC
        // handshake per request.
        {
            let live = self.quic_connection.lock().await.clone();
            if let Some(connection) = live
                && !connection.is_closed()
            {
                return connection.open_stream().await.map_err(Error::from);
            }
        }
        ...
        *self.quic_connection.lock().await = Some(connection);
        Ok(stream)
    }
    ...
    async fn handshake<S: AsyncRead + AsyncWrite + Unpin + ?Sized>(
        ...
        let response_header_byte: u8 = crate::crypto::random_u8();
        ...
        let header = VmessHeader {
            ...
            padding_length: crate::crypto::random_u8() % 16,
    ...
    fn encrypt_aes_gcm(
        ...
        let encrypted = Aes128Gcm::new(key)
    ...
    fn encrypt_chacha20(
        ...
        let encrypted = ChaCha20Poly1305::new(&full_key)
            .encrypt(&nonce_bytes, data, &[])
            .map_err(|e| Error::protocol(format!("Failed to encrypt data: {e:?}")))?;
    ...
    fn decrypt_aes_gcm(
        ...
        let decrypted = Aes128Gcm::new(key)
    ...
    fn decrypt_chacha20(
        ...
        let decrypted = ChaCha20Poly1305::new(&full_key)
            .decrypt(&nonce_bytes, data, &[])
            .map_err(|e| Error::protocol(format!("Failed to decrypt data: {e:?}")))?;
...
impl OutboundProxy for VmessOutbound {
    ...
    async fn disconnect(&self) -> Result<()> {
        let live = self.quic_connection.lock().await.take();
        if let Some(connection) = live {
            connection.close();
        }
        Ok(())
...
fn generate_connection_nonce() -> [u8; 8] {
    let mut nonce = [0u8; 8];
    crate::crypto::fill_random(&mut nonce).expect("Failed to generate nonce");
    nonce
}
...
mod tests {
    ...
    fn test_vmess_outbound_new() {
        ...
        let config = OutboundConfig {
            ...
            outbound_type: crate::core::config::OutboundType::Vmess,
