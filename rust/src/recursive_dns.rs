...
mod tests {
    ...
    fn only_loopback_may_be_reached() {
        ...
        for refused in ["0.0.0.0:0", "[::]:0", "203.0.113.1:0"] {
            let error = start(refused).expect_err("a non-loopback bind must be refused");
            assert!(
    ...
    fn unresolvable_listen_string_is_rejected() {
        ...
        let _ = stop();
        let error = start("not an address").expect_err("garbage must be refused");
        assert!(
    ...
    fn stopping_releases_the_port() {
        ...
        assert_eq!(
            start(&address).expect("the port is free again"),
            address,
