...
- Renamed every layer of the project to Arcadia: the Flutter package (`arcadia`) and every import, the Rust crate (`lib-arcadia`, library stem `rust_lib_arcadia`), the Android application id and Kotlin package (`com.blueokanna.arcadia`, `ArcadiaVpnService`/`ArcadiaTileService` and their JNI symbols), the iOS/macOS bundle identifiers (`com.blueokanna.arcadia`, `Arcadia.app`), the Windows/Linux/HarmonyOS metadata (`arcadia.exe`, `arcadia.desktop`, `com.blueokanna.arcadia`), the bridge channel names (`com.arcadia/proxy`, `com.arcadia/ohos_proxy`), the release assets (`Arcadia-<tag>-…`), the signing secrets and Gradle environment variables (`ARCADIA_*`), the icon source (`assets/arcadia.png`), both READMEs, and all eleven locales.
...
- Collapsed the bridge into a single crate rooted at `rust/`: cargokit (`manifestDir`), the bridge generator (`rust_root`), and the generated dylib loader (`ioDirectory`) all point at that directory instead of the removed `rust/arcadia-lib` sub-crate.
- Restored the pieces the flattened manifest had dropped along the way: the `rust_lib_arcadia` library stem with `rlib`/`cdylib`/`staticlib` outputs (every platform loads the artifact by that name), and the Android-only `jni` and `tracing-android` dependencies that `android_jni.rs` and the logcat layer are compiled against.
...
- Replaced the in-repository proxy core with corduit 0.1.5. `arcadia-core`, `arcadia-dns`, `arcadia-netstack`, and `arcadia-protocol` are gone; the workspace now contains the Flutter Rust Bridge adapter only.
...
- Merged the earlier single-crate iteration back into the bridge workspace: the corduit 0.1.5 bridge stays the engine surface, cargokit and the bridge generator point at `rust/arcadia-lib` again, the Rust toolchain is pinned to 1.97.0 so local builds lint with the same compiler as CI, and the iteration's UI fixes (a proxy-aware IP check that follows the configured mixed port, the `file_picker` 12 reader) plus its CI action bumps and the pub-host lockfile step were kept.
...
- Added live Arcadia process memory reporting with a three-second refresh interval.
